...
public class task_1613 {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        ...
        var weekend = dateTime.getDayOfWeek().getValue();
        if(weekend == FRIDAY && dateTime.getHour() >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR){
            return true;
        }
        if(weekend == SATURDAY){
            return true;
        }
        if (weekend == SUNDAY && dateTime.getHour() < WEEKEND_END_SUNDAY_CUT_OFF_HOUR) {
            return true;
        }
        return false;
    }
